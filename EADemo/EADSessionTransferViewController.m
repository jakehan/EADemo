...
- (IBAction)sendStringButtonPressed:(id)sender;
{
    ...
    if (buf)
    {
        NSLog(@"send text %@",[_stringToSendText text]);
...
#define STRESS_TEST_BYTE_COUNT 10000
    ...
    //for test
//    NSString *testData = @"fwversion1.0";
//    NSData *testD = [NSData dataWithBytes:[testData UTF8String] length:[testData length]];
//
//	[[EADSessionController sharedController] writeData:testD];
}
...
- (void)_sessionDataReceived:(NSNotification *)notification
{
    ...
    while ((bytesAvailable = (uint32_t)[sessionController readBytesAvailable]) > 0) {
        ...
        if (data) {
            ...
            NSString *sendHexString = [[NSString alloc] initWithData:data encoding:NSUnicodeStringEncoding];
            
            NSLog(@"sendHexString received %@",sendHexString);
            
            if(NULL != sendHexString)
            {
                int dec = [sendHexString intValue];
                NSLog(@"sendHexString = %X", dec);
            }
